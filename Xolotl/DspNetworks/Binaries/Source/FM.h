...
template <int NV>
using FM_t_ = container::chain<FM_t_parameters::FM_t_plist<NV>, 
                               wrap::fix<2, xfader_t<NV>>, 
                               split_t<NV>>;

// =========================| Root node initialiser class |=========================

template <int NV> struct instance: public FM_impl::FM_t_<NV>
{
	...
	instance()
	{
		...
		auto& xfader = this->getT(0);                      // FM_impl::xfader_t<NV>
		auto& split = this->getT(1);                       // FM_impl::split_t<NV>
		auto& chain = this->getT(1).getT(0);               // FM_impl::chain_t<NV>
		auto& gain = this->getT(1).getT(0).getT(0);        // core::gain<NV>
		auto& chain1 = this->getT(1).getT(1);              // FM_impl::chain1_t<NV>
...
template <int NV>
using FM = wrap::node<FM_impl::instance<NV>>;
}
